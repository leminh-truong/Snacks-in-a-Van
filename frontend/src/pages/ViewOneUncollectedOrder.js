...
function ViewOneUncollectedOrder({orderid}) {
    ...
    useEffect(()=>{
        ...
        const fetchItem=async () =>{
            try {
            vendorAxios.get(`/van/ready-orders/${orderid}`)
                .then(res => {
    ...
    // Display details of one uncollected order
    return (
        <div>
            {order ? (
            ...
            <button className="centeredButton" onClick={() => routeChange(`/van/ready-orders`)}>Back to Uncollected Orders</button>
        </div>
    )

}

export default ViewOneUncollectedOrder;